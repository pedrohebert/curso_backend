...
class CRUD:
    ...
    def update_task(self, id: int, new_task: dict[str, str | None]) -> int:
        ...
        if id in self.db:
            task: set[tuple[str, str]] = set(
                (key, value) for key, value in new_task.items() if value is not None
            )
...
class Input_handler:
    @staticmethod
    def required_input[T](
        mensage: str, tipo: Callable[[str], T], err_mens: str | None = None
    ) -> T:
        while True:
            try:
...
class ui_handler:
    ...
    def print_all_tesks(tesks: list[tuple[int, dict[str, str]]]):
        ...
        for id, task in tesks:
            ...
            desc_width = max(
                max(len(task["description"]), len("DESCRIPTION")), desc_width
            )
            status_width = max(max(len(task["status"]), len("STATUS")), status_width)

        print(
            f"{'ID':<5} | {'TITLE':<{title_width}} | {'DESCRIPTION':<{desc_width}} | {'STATUS':<{status_width}}"
        )
        print("-" * 55)
...
class Task_gen:
    ...
    def input_id(self):
        return Input_handler.required_input("digite o id da task: ", int)

    def optional_input_status(self) -> str | None:
        opc: dict[int, str] = {1: "To Do", 2: "in progress", 3: "Completed"}
        ...
        while True:
            state: int | None = Input_handler.optional_input(
                "[1-3] ou deixe em branco: ", int
            )
    ...
    def input_create_task(self) -> dict[str, str]:
        task: dict[str, str] = {"title": "", "description": "", "status": ""}
        ...
        task["description"] = Input_handler.required_input(
            "digite a descricao da task: ", str
        )
        clear()
        ui_handler.print_task(task)
    ...
    def input_update_task(self, old_task) -> dict[str, str | None]:
        ...
        task: dict[str, str | None] = {"title": "", "description": "", "status": ""}

        clear()
        ui_handler.print_update(old_task, task)

        task["title"] = Input_handler.optional_input(
            "digite o titulo da tesk ou deixe em branco: ", str
        )
        clear()
        ui_handler.print_update(old_task, task)

        task["description"] = Input_handler.optional_input(
            "digite a descricao da task ou deixe em branco: ", str
        )
        clear()
        ui_handler.print_update(old_task, task)
